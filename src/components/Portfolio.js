...
import arBlog from "../assets/portfolio/arBlog.jpg";
...
const Portfolio = () => {
  const portfolios = [
    ...
    },
    {
      ...
      src: arBlog,
      title: "AR Film Blog",
      description: "A blog platform showcasing augmented reality film reviews.",
      code: "https://github.com/ak397/arfilms-react-master",
      demo: "https://ar-blog-react.netlify.app/",
  ...
  return (
    <div
      name="portfolio"
      className="bg-gradient-to-br from-black via-[#1a0b2e] to-[#1a0b2e] w-full text-white min-h-screen py-20 relative"
    >
      <div
      ...
      <div className="relative z-10 max-w-screen-lg p-4 mx-auto flex flex-col justify-center w-full h-full">
        <div className="pb-8 text-center">
          <p className="text-4xl font-bold inline bg-clip-text text-transparent bg-gradient-to-r from-[#c961de] to-[#7c3aed]">
            Portfolio
          </p>
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-8 px-8 sm:px-0 pb-8">
          {portfolios.map(({ id, src, title, description, demo, code, tags }) => (
            <div
              key={id}
              className="group relative flex flex-col justify-between bg-[#261d45]/40 p-4 rounded-xl border border-[#4a3a7a] hover:border-[#c961de] transition-all duration-300 transform hover:scale-105"
            >
              {/* overlay on hover */}
              ...
              <img
                src={src}
                alt={title}
                className="rounded-lg w-full h-52 object-cover duration-300 group-hover:scale-105 transition-transform shadow-lg shadow-purple-900/20"
              />
              ...
              <div className="flex flex-wrap gap-2 mt-4 mb-3">
                {tags.map((tag, index) => (
                  <span
                    key={index}
                    className="px-2 py-1 text-xs rounded-full bg-[#4a3a7a]/50 text-gray-300 border border-[#4a3a7a] group-hover:border-[#c961de]/50 transition-all duration-300"
                  >
                    {tag}
                  </span>
                ))}
